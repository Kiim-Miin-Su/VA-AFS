...
def get_runtime_device(device_arg):
    if isinstance(device_arg, list):
        first_device = device_arg[0]
    else:
        first_device = device_arg

    first_device = str(first_device).lower()
    ...
    gpu_ids = [int(device) for device in device_arg]
...
def get_parser():
    ...
    parser.add_argument(
        ...
        default=['cpu'],
        nargs='+',
        help='cpu, mps, or CUDA GPU indexes for training/testing')
...
class Processor():
    ...
    def __init__(self, arg):
        ...
        if self.cuda_device_ids and len(self.cuda_device_ids) > 1:
            self.model = nn.DataParallel(
                self.model,
                device_ids=self.cuda_device_ids,
                output_device=self.cuda_device_ids[0])
        if self.arg.ema:
            Model = import_class(self.arg.model)
            self.model_ema = Model(**self.arg.model_args).to(self.torch_device)
    ...
    def load_model(self):
        self.torch_device, self.cuda_device_ids = get_runtime_device(self.arg.device)
        self.output_device = self.torch_device
        Model = import_class(self.arg.model)
        shutil.copy2(inspect.getfile(Model), self.arg.work_dir)
        print(Model)
        self.model = Model(**self.arg.model_args)
        print(self.model)
        self.loss = nn.CrossEntropyLoss().to(self.torch_device)
        ...
        if self.arg.weights:
            ...
            if '.pkl' in self.arg.weights:
                ...
                weights = torch.load(self.arg.weights, map_location=self.torch_device)

            weights = OrderedDict([[k.split('module.')[-1], v.to(self.torch_device)] for k, v in weights.items()])
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (joint, data, label, index) in enumerate(process):
            self.global_step += 1
            with torch.no_grad():
                data = data.float().to(self.torch_device)
                joint = joint.float().to(self.torch_device)
                label = label.long().to(self.torch_device)
        ...
        for ln in loader_name:
            ...
            for batch_idx, (joint, data, label, index) in enumerate(process):
                ...
                with torch.no_grad():
                    ...
                    label = label.long().to(self.torch_device)
                    # for mmd
                    output, y = self.model(data, F.one_hot(label, num_classes=unwrap_model(self.model).num_class), joint)
                    # output, y = self.model(data, F.one_hot(label, num_classes=self.model.num_class))



                    if arg.ema:
                        self.model_ema.to(self.torch_device)
                        output_ema, z_ema = self.model_ema(data, F.one_hot(label, num_classes=unwrap_model(self.model).num_class))
    ...
    def start(self):
        if self.arg.phase == 'train':
            ...
            weights = torch.load(weights_path, map_location=self.torch_device)
            if self.cuda_device_ids and len(self.cuda_device_ids) > 1:
                weights = OrderedDict([['module.'+k, v.to(self.torch_device)] for k, v in weights.items()])
            self.model.load_state_dict(weights)
